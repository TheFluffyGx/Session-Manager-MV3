// js/background.js - corrected MV3 service worker

// get a snapshot of the given window (tabs + groups + active index)
...
async function saveSession(name) {
  ...
  if (existingIndex !== -1) sessions[existingIndex] = snapshot;
...
async function importSessions(sessionsArray, mode = 'merge') {
  ...
  if (mode === 'overwrite') {
    ...
    for (const s of sessionsArray) {
      let name = s.name || `Imported ${new Date(s.createdAt || Date.now()).toLocaleString()}`;
      let attempt = 1;
      while (sessions.find(x => x.name === name)) {
        name = `${s.name || 'Imported'} (${attempt})`;
        attempt++;
      }
      s.name = name;
      sessions.unshift(s);
  ...
  (async () => {
    try {
      ...
      if (msg && msg.type === 'GET_SESSIONS') {
        const store = await chrome.storage.local.get({ sessions: [] });
        sendResponse({ ok: true, sessions: store.sessions });
        return;
      }
